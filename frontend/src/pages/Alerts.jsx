...
export default function Alerts() {
    ...
    const markAsRead = async (id) => {
        ...
        try {
            await api.patch(`/alerts/${id}/read`, {}, session.access_token);
            fetchAlerts();
        } catch (error) {
    ...
    const markAllAsRead = async () => {
        if (!session?.access_token) return;
        try {
            await api.patch('/alerts/read-all', {}, session.access_token);
    ...
    const deleteAlert = async (id) => {
        if (!session?.access_token) return;
        try {
            await api.delete(`/alerts/${id}`, session.access_token);
    ...
    const clearAll = async () => {
        if (!session?.access_token) return;
        try {
            await api.delete('/alerts', session.access_token);
