...
export default function Batches() {
    ...
    const advanceStatus = async (batch) => {
        const next = nextStatus[batch.status];
        if (!next || !session?.access_token) return;
        ...
        try {
            await api.put(`/batches/${batch.id}`, updates, session.access_token);
    ...
    const updateOutputWeight = async (batch, weight) => {
        const outputWeight = parseFloat(weight);
        if (isNaN(outputWeight) || !session?.access_token) return;

        try {
            await api.put(`/batches/${batch.id}`, { output_weight: outputWeight }, session.access_token);
            fetchBatches();
        } catch (error) {
            toast({ title: 'Error', description: error.message, variant: 'destructive' });
        }
